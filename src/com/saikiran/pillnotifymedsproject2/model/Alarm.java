...
public class Alarm extends AbstractModel {
	...
	static String getSql() {
		return Util
				.concat("CREATE TABLE ", TABLE_NAME, " (",
						AbstractModel.getSql(), COL_CREATEDTIME, " INTEGER, ",
						COL_MODIFIEDTIME, " INTEGER, ", COL_NAME, " TEXT, ",
						COL_FROMDATE, " DATE, ", COL_TODATE, " DATE, ",
						COL_RULE, " TEXT, ", COL_INTERVAL, " TEXT, ",
						COL_SOUND, " INTEGER", ");");
	}
	...
	boolean update(SQLiteDatabase db) {
		...
		return db.update(TABLE_NAME, cv, COL_ID + " = ?",
				new String[] { String.valueOf(id) }) == 1 ? true : false;
	}
	...
	public boolean load(SQLiteDatabase db) {
		...
		try {
			if (cursor.moveToFirst()) {
				reset();
				super.load(cursor);
				createdTime = cursor.getLong(cursor
						.getColumnIndex(COL_CREATEDTIME));
				modifiedTime = cursor.getLong(cursor
						.getColumnIndex(COL_MODIFIEDTIME));
				name = cursor.getString(cursor.getColumnIndex(COL_NAME));
				fromDate = cursor
						.getString(cursor.getColumnIndex(COL_FROMDATE));
				toDate = cursor.getString(cursor.getColumnIndex(COL_TODATE));
				rule = cursor.getString(cursor.getColumnIndex(COL_RULE));
				interval = cursor
						.getString(cursor.getColumnIndex(COL_INTERVAL));
				sound = cursor.getInt(cursor.getColumnIndex(COL_SOUND)) == 1 ? true
						: false;
				return true;
			}
			return false;
	...
	public boolean delete(SQLiteDatabase db) {
		...
		db.beginTransaction();
		try {
			db.delete(AlarmTime.TABLE_NAME, AlarmTime.COL_ALARMID + " = ?",
					whereArgs);
			status = db.delete(TABLE_NAME, COL_ID + " = ?", whereArgs) == 1 ? true
					: false;
