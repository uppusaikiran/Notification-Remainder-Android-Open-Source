...
public class AlarmMsg extends AbstractModel {
	...
	static String getSql() {
		return Util.concat("CREATE TABLE ", TABLE_NAME, " (",
				AbstractModel.getSql(), COL_ALARMID, " INTEGER, ",
				COL_DATETIME, " INTEGER, ", COL_STATUS, " TEXT", ");");
	}
	 ...
	 * @param db
	 * @param args
	 *            {alarmId, startTime, endTime, status, orderBy}
	 * @return cursor
	 */
	public static Cursor list(SQLiteDatabase db, String... args) {
		...
		selection += (args != null && args.length > 3 && args[3] != null) ? " AND "
				+ COL_STATUS + " = '" + args[3] + "'"
				: "";
		String orderBy = (args != null && args.length > 4) ? args[4]
				: COL_DATETIME + " DESC";

		return db.query(TABLE_NAME, columns, selection, null, null, null,
				orderBy);
	}
