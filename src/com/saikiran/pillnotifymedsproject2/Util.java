...
public class Util {
	...
	public static String getActualTime(int hour, int minute) {
		if (RemindMe.is24Hours())
			return DbHelper.getTimeStr(hour, minute);

		if (hour < 12)
			return DbHelper.getTimeStr(hour, minute) + " AM";
		else if (hour == 12)
			return DbHelper.getTimeStr(12, minute) + " PM";
		else
			return DbHelper.getTimeStr(hour - 12, minute) + " PM";
	}

	public static String toPersistentTime(String txt) {
		if (txt.contains(" AM"))
			return txt.replace(" AM", "");
		else if (txt.contains(" PM")) {
			String[] tokens = txt.replace(" PM", "").split(":");
			if ("12".equals(tokens[0]))
				return tokens[0] + ":" + tokens[1];
			else
				return (Integer.parseInt(tokens[0]) + 12) + ":" + tokens[1];
		} else
			return txt;
	}
