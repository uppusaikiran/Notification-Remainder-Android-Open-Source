...
import com.saikiran.pillnotifymedsproject2.R;
import com.saikiran.pillnotifymedsproject2.model.Alarm;
import com.saikiran.pillnotifymedsproject2.model.AlarmMsg;
import com.saikiran.pillnotifymedsproject2.model.AlarmTime;
import com.saikiran.pillnotifymedsproject2.model.DbHelper;
...
public class AddAlarmActivity extends Activity {
	...
	protected void onRestoreInstanceState(Bundle state) {
		...
		// datePicker.updateDate(state.getInt("year"), state.getInt("month"),
		// state.getInt("date"));
	...
	private boolean validate() {
		...
		if (vs.getDisplayedChild() == 1) {
			if (TextUtils.isEmpty(fromdateText.getText())) {
				Toast.makeText(this, "Specify from date", Toast.LENGTH_SHORT)
						.show();
				return false;
			}
			if (TextUtils.isEmpty(todateText.getText())) {
				Toast.makeText(this, "Specify to date", Toast.LENGTH_SHORT)
						.show();
				return false;
			}
			try {
				if (sdf.parse(fromdateText.getText().toString()).after(
						sdf.parse(todateText.getText().toString()))) {
					Toast.makeText(this, "From date is after To date!",
	...
	public void create(View v) {
		...
		case 0: // one time
			alarm.setFromDate(DbHelper.getDateStr(datePicker.getYear(),
					datePicker.getMonth() + 1, datePicker.getDayOfMonth()));
			alarmId = alarm.persist(db);

			alarmTime.setAt(DbHelper.getTimeStr(timePicker.getCurrentHour(),
		...
		case 1: // repeating
			alarm.setFromDate(Util.toPersistentDate(fromdateText.getText()
					.toString(), sdf));
			alarm.setToDate(Util.toPersistentDate(todateText.getText()
					.toString(), sdf));
			...
			case R.id.radio1: // interval
				alarm.setInterval(Util.concat(minsEdit.getText(), " ",
						hoursEdit.getText(), " ", daysEdit.getText(), " ",
						monthsEdit.getText(), " ", yearsEdit.getText()));
				break;
			}
			alarmId = alarm.persist(db);

			alarmTime.setAt(Util.toPersistentTime(attimeText.getText()
					.toString()));
			alarmTime.setAlarmId(alarmId);
			alarmTime.persist(db);
			break;
		...
		case DIALOG_TODATE:
			DatePickerDialog.OnDateSetListener dateListener = new DatePickerDialog.OnDateSetListener() {
				public void onDateSet(DatePicker view, int year,
					...
					String txt = DbHelper.getDateStr(year, monthOfYear + 1,
							dayOfMonth);
					try {
	...
	protected void onPrepareDialog(int id, Dialog dialog) {
		...
		case DIALOG_ATTIME:
			if (!TextUtils.isEmpty(attimeText.getText())) {
				String[] arr = Util.toPersistentTime(
						attimeText.getText().toString()).split(":");
		...
		case DIALOG_FROMDATE:
			if (!TextUtils.isEmpty(fromdateText.getText())) {
				String[] arr = Util.toPersistentDate(
						fromdateText.getText().toString(), sdf).split("-");
		...
		case DIALOG_TODATE:
			if (!TextUtils.isEmpty(todateText.getText())) {
				String[] arr = Util.toPersistentDate(
						todateText.getText().toString(), sdf).split("-");
				((DatePickerDialog) dialog).updateDate(
						Integer.parseInt(arr[0]), Integer.parseInt(arr[1]) - 1,
						Integer.parseInt(arr[2]));
			}
